...
const resetForm = () => {
  ...
  newArtwork.value = { 
    id: null, title: '', year: '', series_id: '', description: '', 
    size: '', medium: '', alt_text: '', is_video: false, 
    video_url: '', image_url: '' 
  }
...
const uploadArtwork = async () => {
  ...
  if (videoFileInput.value?.files[0]) fd.append('video_file', videoFileInput.value.files[0])
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-12">
      <div class="md:col-span-1 space-y-6 sticky top-12 h-fit">
        ...
        <div class="space-y-4 bg-zinc-50 p-6 border border-zinc-100 shadow-sm">
          <div>
            <label class="text-[9px] uppercase tracking-widest text-zinc-500 block mb-1">
                Upload Thumbnail {{ isEditing ? '(Optional)' : '' }}
            </label>
            <input type="file" ref="fileInput" class="text-[10px] uppercase w-full border p-2 bg-white mb-2" />
            
            <div class="bg-zinc-200/50 p-2 rounded">
              <label class="text-[8px] uppercase tracking-widest text-zinc-400 block mb-1">Image Path Reference</label>
              <input v-model="newArtwork.image_url" placeholder="Path will appear after upload" class="w-full text-[10px] font-mono bg-transparent focus:outline-none text-zinc-600" />
            </div>
          </div>

          <div class="grid grid-cols-3 gap-2">
            <input v-model="newArtwork.title" placeholder="TITLE" class="col-span-2 border-b p-2 text-sm focus:outline-none bg-transparent" />
            <input v-model="newArtwork.year" placeholder="YEAR" class="col-span-1 border-b p-2 text-sm focus:outline-none bg-transparent" />
          </div>
          ...
          <div class="py-2 border-y border-zinc-200 my-2">
            <label class="flex items-center space-x-2 cursor-pointer">
              <input type="checkbox" v-model="newArtwork.is_video" class="accent-black" />
              <span class="text-[10px] uppercase tracking-widest font-bold">This is a Video Work</span>
            </label>

            <div v-if="newArtwork.is_video" class="mt-4 space-y-3 bg-white p-3 border border-zinc-200">
              <div>
                <label class="text-[8px] uppercase tracking-widest text-zinc-400 block mb-1">YouTube URL</label>
                <input v-model="newArtwork.video_url" placeholder="https://..." class="w-full border-b p-1 text-xs focus:outline-none" />
              </div>
              <div class="text-[8px] text-center text-zinc-300 italic">OR</div>
              <div>
                <label class="text-[8px] uppercase tracking-widest text-zinc-400 block mb-1">Upload .mp4 (Max 10mb)</label>
                <input type="file" ref="videoFileInput" accept="video/mp4" class="text-[9px] w-full" />
              </div>
            </div>
          </div>

          <input v-model="newArtwork.medium" placeholder="MEDIUM" class="w-full border-b p-2 text-sm focus:outline-none bg-transparent" />
          <input v-model="newArtwork.size" placeholder="SIZE" class="w-full border-b p-2 text-sm focus:outline-none bg-transparent" />
          
          <textarea v-model="newArtwork.description" placeholder="DESCRIPTION" class="w-full border p-2 text-sm h-20 focus:outline-none bg-white"></textarea>
      ...
      <div class="md:col-span-2 grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div v-if="art.is_video" class="absolute top-2 right-2 bg-white/90 p-1 rounded-sm shadow-sm">
            <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-3 h-3"><path d="m15.75 10.5 4.72-4.72a.75.75 0 0 1 1.28.53v11.38a.75.75 0 0 1-1.28.53l-4.72-4.72M4.5 18.75h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25h-9A2.25 2.25 0 0 0 2.25 7.5v9a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
          </div>
          ...
          <div class="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity p-4 flex flex-col justify-between">
            <div>
              ...
              <p v-if="art.year" class="text-zinc-500 text-[8px]">{{ art.year }}</p>
            </div>
